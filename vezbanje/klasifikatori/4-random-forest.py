...
modified_data = [[val for i, val in enumerate(row) if i != col_index] for row in dataset]

X = [row[:-1] for row in modified_data]
Y = [row[-1] for row in modified_data]

classifier = RandomForestClassifier(criterion=criterium, random_state=0, n_estimators=treecount)

split = len(modified_data) * 85 // 100
...
prediction = classifier.predict([record_modified])
print(prediction[0])
probabilites = classifier.predict_proba([record_modified])
