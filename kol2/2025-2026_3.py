...
if __name__ == '__main__':
    ...
    for i in range(3):
        test = subsets[i]
        train = [row for j in range(3) if j != i for row in subsets[j]]
        ...
        # forest
        forest = RandomForestClassifier(n_estimators=N, criterion='gini', random_state=0, )
        forest.fit(train_X, train_Y)
        acc = forest.score(test_X, test_Y)
        acc_forest.append(acc)

        # mumbo jumbo
        naive = GaussianNB()
        naive.fit(train_X, train_Y)
        tree = DecisionTreeClassifier(criterion='gini', max_depth=M, random_state=0)
        tree.fit(train_X, train_Y)

        p1 = naive.predict_proba(test_X)
        p2 = tree.predict_proba(test_X)

        classes = naive.classes_


        hits = 0
        for naive_pred, tree_pred, truth in zip(p1, p2, test_Y):
            max1 = naive_pred.max()
            max2 = tree_pred.max()
            if max1 >= max2:
                index = naive_pred.argmax()
            else:
                index = tree_pred.argmax()
            pred = classes[index]
            if pred == truth:
                hits+=1
        acc_kire = hits/len(test_Y)
        acc_ensamble.append(acc_kire)

    acc1 = sum(acc_forest)/3
    acc2 = sum(acc_ensamble)/3

    print(f'''Accuracy with random forest: {acc1}
Accuracy with naive bayes and decision tree: {acc2}''')
