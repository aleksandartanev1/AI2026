...
class Box(Problem):
    ...
    def goal_test(self, state):
        _, box1, box2 = state
        return box1 == box2 == self.end_point


    @staticmethod
    def check_valid(state):
        manpos, box1, box2 = state

        if not (0 <= box1[0] < 5 and 0 <= box1[1] < 5):
            return False

        if not (0 <= box2[0] < 5 and 0 <= box2[1] < 5):
            return False

        if box1 == box2 and box2 != (4,4):
            return False

        if box2 == box1 and box1 != (4,4):
            return False
    ...
    def successor(self, state):
        ...
        for action, (nx, ny) in directions.items():
            ...
            if  0 <= newmanx < 5 and 0 <= newmany < 5:
                if (newmanx, newmany) == box1 and self.check_valid(((newmanx, newmany), (newbox1x, newbox1y), box2)):
                    succ[f'Push box 1 {action}'] = ((newmanx, newmany), (newbox1x, newbox1y), box2)
                if (newmanx, newmany) == box2 and self.check_valid(((newmanx, newmany), box1, (newbox2x, newbox2y))):
                    succ[f'Push box 2 {action}'] = ((newmanx, newmany), box1, (newbox2x, newbox2y))
                if (newmanx, newmany) not in (box1, box2):
                    succ[f'Move man {action}'] = ((newmanx, newmany), box1, box2)
...
if __name__ == '__main__':
    ...
    resultt = breadth_first_graph_search(box)

    if resultt:
        print(resultt.solution())
    else:
        print("No Solution!")