...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(5):
                if squaress[i] == self.goal[i]:
                    continue

                for direction, (dx, dy) in directions.items():
                    squarex, squarey = squaress[i]
                    newx = squarex + dx
                    newy = squarey + dy
                    newpos = (newx, newy)
                    if self.check_valid((squaress[:i] + (newpos,) + squaress[i+1:])):
                        action = f'Move square {i + 1} {direction}'
                        newstate = squaress[:i] + (newpos,) + squaress[i+1:]
