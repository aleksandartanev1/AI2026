...
class Snake(Problem):
    def __init__(self, snakehead, snakebody, direction, green, red):
        initial_state = (snakehead, snakebody,direction, tuple(green), tuple(red))
    ...
    def goal_test(self, state):
        head, body, facing, green, red = state
    ...
    def successor(self, state):
        ...
        if facing == 'east':
            forward = ((head[0] + 1, head[1]), body, facing, green, red)
            leftturn = ((head[0], head[1] + 1), body, 'north', green, red)
            rightturn = ((head[0], head[1] - 1), body, 'south', green, red)
        elif facing == 'west':
            forward = ((head[0] - 1, head[1]), body, facing, green, red)
            leftturn = ((head[0], head[1] - 1), body, 'south', green, red)
            rightturn = ((head[0], head[1] + 1), body, 'north', green, red)
        elif facing == 'south':
            forward =  ((head[0], head[1] - 1), body, facing, green, red)
            leftturn = ((head[0] + 1, head[1]), body, 'east', green, red)
            rightturn = ((head[0] - 1, head[1]), body, 'west', green, red)
        elif facing == 'north':
            forward = ((head[0], head[1] + 1), body, facing, green, red)
            leftturn = ((head[0] - 1, head[1]), body, 'west', green, red)
            rightturn =  ((head[0] + 1, head[1]), body, 'east', green, red)

        directions = {
            "Move forward":forward,
            "Turn right":rightturn,
            "Turn left":leftturn
        }


        for movement, newstate in directions.items():
            newhead, newbody, newdirection, newgreen, red = newstate

            headx, heady =  newhead

            if 0 <= headx < 10 and 0 <= heady < 10:
                if newhead not in red and newhead not in newbody:
                    if newhead in green:
                        newbody = list(newbody)
                        newbody.append(head)
                        newbody = tuple(newbody)
                        newgreen = list(newgreen)
                        newgreen.remove(newhead)
                        newgreen = tuple(newgreen)
                        newstate = (newhead, newbody, newdirection, newgreen, red)
                        succ[movement] = newstate

                    if newhead not in green:
                        newbody = list(newbody)
                        newbody.pop(0)
                        newbody.append(head)
                        newbody = tuple(newbody)
...
if __name__ == '__main__':
    ...
    problem = Snake(snakehead, snakebody, direction, greenapples, redapples)
