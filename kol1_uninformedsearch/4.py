...
class Checkers(Problem):
    def __init__(self, balls, obstacles, size):
        initial_state = (tuple(balls), tuple(obstacles))
    ...
    def goal_test(self, state):
        bs, os = state
        target = (self.size - 1,  self.size // 2)
    ...
    def successor(self, state):
        ...
        for balll in balls:
            ...
            for direction, (dx, dy) in directions.items():
                    ...
                    ny = y + dy
                    newball = (nx, ny)
                    if newball in balls and newball not in obstacles and 0 <= nx < self.size and 0 <=ny < self.size:
                        spx = nx + dx
                        spy = ny + dy
                        space = (spx, spy)
                        if space not in balls and space not in obstacles and 0 <= spx < self.size and 0 <= spy < self.size:
                            new_balls = list(balls)
                            new_balls.remove(newball)
                            new_balls.remove(balll)
                            new_balls.insert(0, space)
                            new_balls = tuple(new_balls)
                            action = f'{direction} (x={x},y={y})'
                            newstate = (new_balls, obstacles)
                            succ[action] = newstate
...
if __name__ == "__main__":
    ...
    problem = Checkers(balls, obstacles, size)
